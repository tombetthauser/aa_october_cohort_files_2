...
#  N+1 solution (half credit):
  def n_plus_one_count_of_big_actors_roles
    actors = self.actors

    actors_count = {}
      actors.each do |actor|
        actors_count[actor.name] = actor.movies.length
      end

    actors_count.select!{|k, v| v > 10}
  end

# Front loaded query approach (full credit):
  def count_of_big_actors_roles
    actors = self
      .actors
      .includes(:movies)
